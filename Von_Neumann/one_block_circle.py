...
MOVES = [(0, 1),(1, 0),(0, -1),(-1, 0)]  # Diagonal

# Function to generate a circle formation with the specified pattern
def generate_circle_shape(num_agents):
    ...
    for row_idx, agents_in_row in enumerate(row_pattern):
        ...
        if row_idx in rows_with_gaps:
            ...
            # For the original pattern, it's 2 on each side
            # As we add agents, keep the gap in the middle and add agents symmetrically
...
# Set the title once
containers['title'].title("Moore Method - Circle Formation")
...
def initialize_agents(num_agents):
    ...
    # Generate target positions as a circle
...
if 'target_viz_created' not in st.session_state:
    ...
    containers['target_viz'].write("### Target Circle Formation")
    containers['target_viz'].pyplot(target_fig)
    plt.close(target_fig)
    
    # Show circle info
    radius = min(GRID_SIZE // 2 - 1, max(2, int(math.sqrt(st.session_state.current_agent_count / math.pi))))
    containers['formation_info'].write("### Circle Structure:")
    containers['formation_info'].write(f"Approximate Radius: {radius}")
    containers['formation_info'].write(f"Total agents: {st.session_state.current_agent_count}")
