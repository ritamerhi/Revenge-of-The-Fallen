...
# MODIFIED: Function to determine inverted triangle dimensions based on number of agents
def calculate_triangle_dimensions(num_agents):
    # For an inverted triangle, the first row has 2 agents
    # Each subsequent row has 2 more agents than the previous row
    # So row i has 2 + 2*i agents (row 0 has 2, row 1 has 4, row 2 has 6, etc.)
    # The total agents in n rows is: 2*n + 2*sum(i) for i from 0 to n-1
    # This simplifies to: 2*n + 2*n*(n-1)/2 = 2*n + n*(n-1) = n*(n+1)
    
    # We need to find n such that n*(n+1) >= num_agents
    n = 1
    while n * (n + 1) < num_agents:
        n += 1
    
    return n  # n is the number of rows in the triangle

# MODIFIED: Function to generate an inverted triangle formation
def generate_triangle_shape(num_agents):
    # Calculate how many rows we need for the triangle
    num_rows = calculate_triangle_dimensions(num_agents)
    
    # Position the top of the triangle near the top of the grid
    start_row = 2  # Leave a little space at the top
    ...
    # Generate positions for the inverted triangle (wide at the bottom)
    agent_count = 0
    for i in range(num_rows):
        # Each row i has 2 + 2*i elements (row 0 has 2, row 1 has 4, row 2 has 6, etc.)
        row_size = 2 + 2 * i
        
        # Center each row
        start_col = (GRID_SIZE - row_size) // 2
        ...
        for j in range(row_size):
            if agent_count < num_agents:
                ...
                # Store position
                if agent_count in problematic_indices:
    ...
    # This ensures they get more favorable positions in the triangle
...
# Plan coordinated moves function
...
if 'ui_containers' not in st.session_state:
    st.session_state.ui_containers = {
        ...
        'progress': st.empty(),
        'message': st.empty(),
        'shape_info': st.empty(),  # Changed from 'square_info' to 'shape_info'
        'target_viz': st.empty(),
        'stats': st.empty(),
        'swap_info': st.empty(),
        'deadlock_info': st.empty(),  
    }
...
# Set the title once
containers['title'].title("Agent Formation Control - Triangle")  # Updated title
...
def initialize_agents(num_agents):
    ...
    # Generate target positions (CHANGED: use triangle formation)
